...
short reference = 0;
short sign = 0;
static unsigned int needaref = 0;
...
void printHelp(unsigned char str[1500]){
    ...
    strcat(str,  "\"ref\"  => changes the refrence                                     \n\r");
    ...
    if (rxCnt != 0 ){
        ...
        if(rxBuffer[rxCnt-1] != 13 && rxBuffer[rxCnt-1] != 27 && rxBuffer[rxCnt-1] != '\b'){
            ...
            // check if "new line (ENTER): (DEC) 13" was pressed and match a possible command. If no command matched, print a warning message. Rise the "flush" flag anyway
                ...
                if (!strcmp(outputBuffer, "info")){
                ...
                else if (!strcmp(outputBuffer, "ref")){
                    strcpy(outputBuffer,"\n\r");
                    flushTheBufferIsRequired = 1;
                    needaref = 1;
                    return 16; // command "ref" hit
                }
                else if((isdigit(outputBuffer[0])||((outputBuffer[0] == '-')&&isdigit(outputBuffer[1]))) && needaref == 1){
                    needaref = 0;
                    i = 0;
                    dec = 0;
                    sign = 1;
                    ...
                    while(isdigit(outputBuffer[i])) {
                        dec = dec * 10 + ( outputBuffer[i] - '0' );
                        i++;
                    }
                    reference = sign*dec;
                    sprintf(outputBuffer, "\n\rVous avez entre %d\n\r", reference);
                    flushTheBufferIsRequired = 1;
                    return 0; // command "cons" hit
                }
                else{
